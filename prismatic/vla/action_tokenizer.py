...
class DDActionTokenizer:
    def __init__(self, bins: int = 256, min_action: float = -1.0, max_action: float = 1.0):
        ...
        # 257 edges -> 256 intervals -> 256 centers
        self.edges = np.linspace(min_action, max_action, bins + 1)
        self.centers = (self.edges[:-1] + self.edges[1:]) / 2.0  # length = 256

    def encode(self, actions: np.ndarray) -> np.ndarray:
        x = np.clip(actions, self.min_action, self.max_action)
        # digitize in [1, n_bins], then -1 -> [0, n_bins-1]
        q = np.digitize(x, self.edges, right=False) - 1
        return np.clip(q, 0, self.n_bins - 1).astype(np.int64)

    def decode(self, token_ids: np.ndarray) -> np.ndarray:
        q = np.clip(token_ids, 0, self.n_bins - 1)
        return self.centers[q]
